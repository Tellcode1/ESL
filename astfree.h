...
e_asnode_free(e_ast* p, int nodeID)
{
  ...
  switch (E_GET_NODE(p, nodeID)->type) {
    case E_ASNODE_ROOT:
      for (int i = 0; i < E_GET_NODE(p, nodeID)->val.root.nexprs; i++) e_asnode_free(p, E_GET_NODE(p, nodeID)->val.root.exprs[i]);
    ...
    case E_ASNODE_EXPRESSION_LIST:
      for (int i = 0; i < E_GET_NODE(p, nodeID)->val.exprs.nexprs; i++) e_asnode_free(p, E_GET_NODE(p, nodeID)->val.exprs.exprs[i]);
      free(E_GET_NODE(p, nodeID)->val.exprs.exprs);
      break;
    case E_ASNODE_FOR: break;
    case E_ASNODE_WHILE:
      for (int i = 0; i < E_GET_NODE(p, nodeID)->val.while_stmt.nexprs; i++) e_asnode_free(p, E_GET_NODE(p, nodeID)->val.while_stmt.exprs[i]);
    ...
    case E_ASNODE_FUNCTION_DEFINITION: {
      for (int i = 0; i < E_GET_NODE(p, nodeID)->val.func.nexprs; i++) e_asnode_free(p, E_GET_NODE(p, nodeID)->val.func.exprs[i]);
      for (int i = 0; i < E_GET_NODE(p, nodeID)->val.func.nargs; i++) free(E_GET_NODE(p, nodeID)->val.func.args[i]);
    ...
    case E_ASNODE_IF: {
      ...
      for (int i = 0; i < E_GET_NODE(p, nodeID)->val.if_stmt.nstmts; i++) e_asnode_free(p, E_GET_NODE(p, nodeID)->val.if_stmt.body[i]);
      for (int i = 0; i < E_GET_NODE(p, nodeID)->val.if_stmt.nelse_stmts; i++) e_asnode_free(p, E_GET_NODE(p, nodeID)->val.if_stmt.else_body[i]);
      for (int i = 0; i < E_GET_NODE(p, nodeID)->val.if_stmt.nelse_ifs; i++) {
        struct e_if_stmt* else_if = &E_GET_NODE(p, nodeID)->val.if_stmt.else_ifs[i];
        e_asnode_free(p, else_if->condition);
        for (int j = 0; j < else_if->nstmts; j++) e_asnode_free(p, else_if->body[j]);
        free(else_if->body);
      }
    ...
    case E_ASNODE_CALL:
      ...
      for (int i = 0; i < E_GET_NODE(p, nodeID)->val.call.nargs; i++) e_asnode_free(p, E_GET_NODE(p, nodeID)->val.call.args[i]);
