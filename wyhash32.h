...
/**
 * I (Tellcode) added the header guard and my formatter formatted the code.
...
#endif
static inline unsigned
_wyr24(const uint8_t* p, unsigned k)
{ return (((unsigned)p[0]) << 16) | (((unsigned)p[k >> 1]) << 8) | p[k - 1]; }
static inline void
_wymix32(unsigned* A, unsigned* B)
{
...
wyhash32(const void* key, uint64_t len, unsigned seed)
{
  ...
  if (i >= 4) {
    seed ^= _wyr32(p);
    see1 ^= _wyr32(p + i - 4);
  } else if (i) seed ^= _wyr24(p, (unsigned)i);
  _wymix32(&seed, &see1);
  _wymix32(&seed, &see1);
  return seed ^ see1;
}
